...
Using the rigid body configuration error $\bodyHomoCoordE{}{} = \bodyHomoCoordR{}{}^{-1} \bodyHomoCoord{}{}$ and its velocity $\bodyVelE{}{} = \veeOp\big(\bodyHomoCoordE{}{}^{-1} \bodyHomoCoordEd{}{}\big) = \bodyVel{}{} - \Ad{\bodyHomoCoordE{}{}}^{-1} \bodyVelR{}{}$ we modify the energies from \autoref{sec:CtrlApproachParticles} to
...
% \end{align} 
% \end{subequations}
Note that the stiffness potential and force are identical to the previous, particle based approach.
The dissipation function and corresponding force differ from the particle based approach, but coincide with the model of a body moving inside a viscous fluid, see \autoref{sec:DampingSE3} and \eqref{eq:RBDissFkt}.
...
\begin{align}\label{eq:ClosedLoopRBApproach2}
 \bodyHomoCoordEd{}{} = \bodyHomoCoordE{}{} \wedOp(\bodyVelE{}{}),
\qquad
...
\end{align}
A quite similar (though restricted to $\bodyCOM{}{}=\bodyCOD{}{}=\bodyCOS{}{}=\tuple{0}$) closed loop for the free rigid body is proposed in \cite{Koditschek:TotalEnergy}, but motivated and derived very differently.
...
The remaining solutions are the constant ($\bodyVelE{}{} = \tuple{0}$) configurations $\bodyHomoCoordE{}{} \neq \idMat[4]$ which are critical points of the potential $\potentialEnergyC$, see \autoref{sec:AppendixAttitudePotential}.
...
The change of the total energy $\totalEnergyC = \kineticEnergyC + \potentialEnergyC$ along the solutions of \eqref{eq:CtrlApproachBody} does \textit{not} take a similar form as for the free rigid body \eqref{eq:CtrlTotalEnergyRBAppraoch2}.
Consequently there is no simple conclusion about stability.
For a constant reference $\sysCoordR = \const$ this approach \eqref{eq:CtrlApproachBody} coincides with the particle based approach \eqref{eq:CtrlApproachParticles} and, as shown above, $\totalEnergyC$ is a Lyapunov function.
