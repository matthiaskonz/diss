...
%  = \tfrac{1}{2} \displaystyle \sumParticles \tfrac{1}{\particleMassC{\PidxI}} \norm{\particleMassC{\PidxI} \particlePosdd{\PidxI} - \particleForceImpressed{\PidxI}}^2
 ...
 \end{array}.
\end{align}
Note that we also replaced the particle masses $\particleMass{\PidxI}$ by \textit{desired masses} $\particleMassC{\PidxI}$ as additional design parameters.
...
It is, however, for the special case of $\sysVelR = \tuple{0}$, i.e.\ proves stability of a constant reference configuration. 
...
\begin{subequations}\label{eq:CtrlApproachParticleRBEnergies}
\begin{align}
...
\end{align}
\end{subequations}
The closed loop kinetic equation is $\genForceInertiaC + \genForceDissC + \genForceStiffC = \tuple{0}$ and the actual controller can be solved by combining this with the model kinetics.
The total energy $\totalEnergyC = \kineticEnergyC + \potentialEnergyC$ can be obtained from \eqref{eq:CtrlApproachParticleRBEnergies}.
Even though it contains more structure than the general approach above, it still is not a Lyapunov function for arbitrary reference trajectories.

\fixme{
The closed loop contains 30 tuning parameters within the matrices $\bodyInertiaMatCp{}{}$, $\bodyDissMatCp{}{}$, $\bodyStiffMatCp{}{} \in \SymMatP(4)$.
...
The potential energy from \eqref{eq:SolCtrlGaussPrinciple} may be formulated as $\potentialEnergyC = \sum_{\BidxII=1}^{\numRigidBodies} \tfrac{1}{2} \norm[\bodyStiffMatCp{0}{\BidxII}]{\big(\bodyHomoCoord{0}{\BidxII} - \bodyHomoCoordR{0}{\BidxII}\big)^\top}^2$ with a body stiffness matrices $\bodyStiffMatCp{0}{\BidxII}$ for each body.
...
This approach will be applied to several examples later in this chapter.
