...
The bicopter considered here is a single rigid body with two tiltable propellers as illustrated in \autoref{fig:BicopterModel}.
With the same coordinates as in the previous examples, the equations of motion are identical except for the generalized force from the propellers.
\begin{align}
 \genForceInput
 =
...
The transformation of the actual control inputs $\PropForce[1]$, $\PropForce[2]$ and $\PropTilt[1]$, $\PropTilt[2]$ to an auxiliary input $\sysInput$ is used to achieve the linear form $\genForceInput = \sysInputMat\sysInput$.
...
$\epsx\neq\epsy$, this system is probably not flat; parameterization of a feasible reference trajectory is not trivial.

\paragraph{Closed loop template.}
As for the quadcopter, the closed-loop templates are those of a free rigid body, i.e.\ \autoref{sec:CtrlApproachParticlesSingleBody}, \autoref{sec:CtrlApproachBodySingleBody} and \autoref{sec:CtrlApproachEnergySingleBody}.
...
Thus we set $\Jcxy=\Jcxz=\Jcyz=0$, $\scx=\scy=0$, with the same assumptions for damping and stiffness.
...
Note that, in contrast to the quadcopter, $\epsx \neq \epsy$ implies $\Jcx \neq \Jcy$, and since the remaining relevant parameters are identical, the closed-loop dynamics for x and y are different from one another.
...
Since the bicopter model is probably not flat, generating a feasible reference trajectory is not trivial.
...
As the initial condition for the simulation, we chose a position error of $0.5\,\unit{m}$ in all directions and a heading error of $90^\circ$.
This guarantees that all parts of the controller are excited.
...
Roughly during the first loop (the first $2\,\unit{s}$), the state trajectory converges to its reference and tracks it for the remaining simulation time.
This is also quantified by the error energy $\totalEnergyC$ in the bottom of \autoref{fig:BicopterCircleSimRes}.
There, one can also see that $\totalEnergyCd \neq \accCtrl{\mathcal{P}}$, which is most notable during the time around $t=0.5\,\unit{s}$ when input constraints are active, but also due to the non-vanishing matching force $\tilde{\sysForce}$.
Nevertheless, if we neglect the parts where the input constraints are active, the error energy converges to zero.
