...
For many applications, it is desirable to \textit{decouple} the translational and the rotational motion of the body.
%What this means and how it is achieved is discussed in this subsection.

Observing the closed loop equations one can see immediately that the coupling terms vanish if $\sc=\lc=\hc=\tuple{0}$, i.e.\ the chosen body fixed point $\r$ coincides with the centers of mass, damping, and stiffness.
Then the rotational dynamics are identical to the closed loop given in \autoref{sec:CtrlExampleRigidBodyOrientation} (\autoref{sec:CtrlExampleRevoluteJoint} for the planar case), and are indeed decoupled from the translational motion.

\paragraph{Translational dynamics.}
The translational dynamics are more intricate.
...
The rotational energies for the corresponding approaches coincide with those given in \autoref{sec:CtrlExampleRigidBodyOrientation} (\autoref{sec:CtrlExampleRevoluteJoint} for the planar case).
...
The crucial observation is that for all approaches the translational dynamics and energy are independent of the actual orientation $\R$ and angular velocity $\w$, but some approaches depend on the reference quantities $\RR$ and $\wR$.
For a constant reference orientation $\RR = \const$ and consequently $\wR = \tuple{0}$, all four approaches are equivalent.
Furthermore, note that the error dynamics and the energies are invariant to the position reference trajectory $t\mapsto\rR(t)$.
...
The difference between these cases is discussed using simulation results for the simpler, yet illustrative, planar rigid body example:
...
Viewed from the inertial frame (top left of \autoref{fig:RBCtrlPosError}), approach 2 yields a straight-line path to the reference position, whereas the other approaches spiral around it.
Viewed from the reference frame (top right of \autoref{fig:RBCtrlPosError}), approach 3 follows a direct path given the initial velocity.
...
The rates of convergence for approaches 2 and 3 are the same, as expected from their identical characteristic polynomial.

Even though the energy-based approach with the transport map from \eqref{eq:TimeDerivativeRigidBodxPotential} may be mathematically the most elegant solution, its simulation result is less intuitive.
Which approach is most desirable depends on the application.
For indoor robots (like the multicopters discussed in the next chapter), it is probably most desirable for it to correct position error along a straight line in the inertial frame.
