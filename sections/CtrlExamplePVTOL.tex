...
The planar vertical take-off and landing aircraft (PVTOL), as depicted in \autoref{fig:PVTOLModel}, is a common benchmark problem discussed in, e.g., \cite{Hauser:PVTOL} and \cite{Fliess:LieBacklund}.
\begin{figure}
 \centering
...
Here, we choose the body-fixed frame to be positioned at the center of mass, i.e.\ $\sx = \sy = 0$.
...
For the following we choose $\sysInputMatLComp = [1, 0, -\tfrac{\sin\beta}{l \cos\beta}]$ as a left complement to $\sysInputMat$.
...
As for the model, the closed-loop templates are the same as those of the planar rigid body from \autoref{sec:CtrlPlanarRigidBody}.
For symmetry reasons, we set the parameters $\hcx=\lcx=\scx=0$.
% \begin{align}
...
Instead, we will investigate its linear approximation about any reference trajectory with $\tiltAngle_\idxRef = 0$.
The matrices of the linearized model and desired closed loop are
\begin{subequations}
\begin{align}
...
From a stability and attractiveness point of view, this is necessary: as there is no inherent rotational dissipation ($\sigcz = 0$), the corresponding energy is dissipated through its coupling to the translational motion.
The coupling also reflects the characteristic behavior of the PVTOL: tilting is required for sideways motion.



\paragraph{Pole placement.}
While the tuning parameters may be mechanically intuitive, a classic pole-placement approach can be instructive.
...
\end{multline}
From a desired polynomial of fourth degree $\EigenVal^4 + p_3 \EigenVal^3 + p_2 \EigenVal^2 + p_1 \EigenVal + p_0$, we get the parameters
\begin{align}
...
Therein one finds two exemplary simulation results: one for stabilization and tracking, and one for a looping trajectory. 

The PVTOL is a simpler version of the 3D bicopter, which will be discussed in the following; the simulation results there also apply to the PVTOL.
