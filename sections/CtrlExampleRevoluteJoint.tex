...
\paragraph{Model.}
Another elementary case is the revolute joint, i.e. a rigid body constrained to rotate about an axis as illustrated on the left side of \autoref{fig:RevoluteJoint}.
Using the joint angle $\jointAngle$, the rigid body configuration may be written as
\begin{align}
...
The potential can be realized by attaching a single linear spring with stiffness $\sfrac{\kapczz}{\hcz^2}$ between the desired configuration and the actual configuration at a distance $\hcz$, as illustrated on the right side of \autoref{fig:RevoluteJoint}.
...
\end{align}
The total energy $\totalEnergyC = \kineticEnergyC + \potentialEnergyC$, $\totalEnergyCd = -2\dissFktC$ can be used to conclude that the system converges for \textit{almost} all initial conditions $(\jointAngleE(0), \jointAngleEd(0))$.
The remaining initial condition $\jointAngleE(0) = \pm\pi$ and $\jointAngleEd(0)=0$ is an unstable equilibrium, see \autoref{fig:PhasePlotSO2}.
...
\end{align}
The difference between the closed loop \eqref{eq:RevoluteJointClosedLoopApproachEnergy} and \eqref{eq:RevoluteJointCtrlLin} can be visualized by the corresponding phase plots in \autoref{fig:PhasePlotSO2}.
The linear control law leads to non-smooth phase curves at $\jointAngle = \pm\pi$, which is a consequence of the linear design for a system whose configuration space is actually $\Sphere^1$, not $\RealNum$.
