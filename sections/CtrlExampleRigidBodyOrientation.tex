...
Using the angular velocity $\w = \veeMatOp(\R^\top\Rd)$ as velocity coordinates, and denoting the inertia matrix by $\J$ and the control torques about the body-fixed axes by $\tuple{\tau}$, the equations of motion may be written as
\begin{align}
 \Rd = \R \wedOp(\w)
\qquad
...
Using the attitude error $\RE = \RR^\top \R$, the error potential, its differential, and its Hessian are
\begin{subequations}
\begin{align}
...
The total energy coincides with that of the body-based approach.
The two approaches differ only in the gyroscopic terms.
% \begin{align}
%  \gyroForceC 
...
As proposed in \autoref{sec:CtrlLinearization}, for small errors we write $\R = \RR(\idMat + \wedOp(\LinErrorCoord))$ and $\w = \wR + \LinErrorCoordd$.
If we additionally assume that $\wR$ is small, the linear approximations for the closed loop of each approach coincide as
\begin{align}
 \Jc \LinErrorCoorddd + \sigc \LinErrorCoordd + \kapc \LinErrorCoord = 0.
\end{align}
The desired controlled inertia matrix $\Jc$ should be an intuitive parameter for tuning.
The desired controlled damping matrix $\sigc$ and stiffness matrix $\kapc$ might not be as established, but as shown above, should be just as intuitive. 