...
However, this illustrates some benefits of the proposed control approach: one does not have to inspect the full system inertia matrix but only the much less cumbersome body inertia matrices to conclude, for example, stability of the closed loop.
...
A reasonable choice of system coordinates is the joint angles $\sysCoord = [\jointAngle[1], \ldots, \jointAngle[6]]^\top$ with trivial kinematics $\sysVel = \sysCoordd$.
...
\nonumber\\
 ...
 .
\end{align}
This, together with the body inertia matrices, the gravity vector $\gravityAcc$, and the control forces $\sysInput = [\tau_1, \ldots, \tau_6]^\top$, determines the equations of motion.

\paragraph{Controller parameterization 1: Joint space control.}
As above, we consider two different sets of controller parameterizations.
...
The resulting closed-loop kinetics are six decoupled equations identical to those for the SCARA \eqref{eq:ScaraClosedLoop11} and \eqref{eq:ScaraClosedLoop12}.

\paragraph{Controller parameterization 2: Work space control.}
As a second case, consider that for many applications the task of the robot arm is to control the position and orientation of a tool mounted at the end of its kinematic chain.
This tool might have a particularly meaningful center point (TCP) and principal axes.
...
\end{align}
If the term in the brackets vanishes, it means that the wrist lies on the axis of $\jointAngle[1]$.
The condition $\cos\jointAngle[3] = 0$ occurs when the arm is completely straight; this is the singularity we already encountered with the SCARA robot.
The last three axes with angles $\jointAngle[4]$, $\jointAngle[5]$, $\jointAngle[6]$ can be regarded as Euler angles in the sequence XYX, and $\sin\jointAngle[5] = 0$ is their singularity.
...
The behavior of the two different parameterizations is analogous to the two parameterizations of the SCARA robot.
Which one is more suitable depends on the actual control task.
Furthermore, the two presented parameterizations are just special cases of the dynamics that can be achieved with the more general control approach presented in this work.
