...
The remaining two axes are sufficient to position a tool (red point in \autoref{fig:Scara}) in the workspace (green shaded area).
...
\paragraph{Model.}
The model consists of two rigid bodies constrained by two revolute joints.
A reasonable choice of coordinates is the relative joint angles $\sysCoord = [\jointAngle[1], \jointAngle[2]]^\top$ and their derivatives $\sysVel = [\jointAngled[1], \jointAngled[2]]^\top$.
...
\end{align}
These parameters are directly associated with the joint-angle errors $\jointAngleE[i] = \jointAngle[i] - \jointAngleR[i],\; i=1,2$.
...
\end{align}
It obeys the transport map $\sysTransportMap = \idMat[2]$.
...
The closed loop kinetics for the particle- and energy-based approaches, in terms of the model coordinates $\sysCoord$ and the error velocity $\sysVelE = \sysVel - \sysTransportMap\sysVelR$, are
\begin{subequations}
\begin{multline}
...
\end{align}
\end{subequations}
With the body-based approach we obtain a similar closed loop that is not displayed or discussed here.
...
This means that one cannot compute the control law if $\sin\jointAngle[2] = 0$.
Recalling the mechanical model of the SCARA \autoref{fig:Scara}, this singularity is evident from a geometric point of view:
If $\sin\jointAngle[2] = 0$, the tool can only move tangentially to the boundary of the workspace but not radially.
...
It is rather an intrinsic singularity resulting from forcing dynamics suitable for $\RealNum^2$ on a system that has the configuration space $\Sphere^2$.
...
\autoref{fig:ScaraSimRes} and \autoref{fig:ScaraSimSnapshots} show simulation results for the SCARA robot with the two proposed parameterizations.
...
The crucial difference between the two parameterizations is that, although the tool position $\toolPos$ tracks its reference in both cases, the joint angles $\jointAngle[1], \jointAngle[2]$ do not track their references for the second parameterization.
