...
two bodies in prismatic joints connected by a linear spring, where only one is directly actuated by the force $F$, as illustrated in \autoref{fig:TwoMassSpring}.
\begin{figure}[ht]
 \centering
...
\end{align}
Since this equation is linear in the system coordinates, we can separate it into
\begin{subequations}
\begin{align}
...
It can be resolved by acknowledging that $z_2$ is a \textit{flat output} of the system and planning the reference trajectory accordingly, i.e.
\begin{align}
...
However, this approach may give additional \textit{physical} insight into the resulting closed-loop system.
For example, the closed-loop system must have an inertial coupling ($\bodyMassC{1}{2} \neq 0$) of the two bodies if one wants to tune all four poles.
