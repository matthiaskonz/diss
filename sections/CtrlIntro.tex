...
First, we will restrict to fully-actuated systems and later try to expand the approach to underactuated systems.
...
General Lyapunov designs can be found in \cite{OlfatiSaberDiss} and an approach called \textit{controlled Lagrangian} is proposed in \cite{bloch2000controlled}.
...
However, for systems whose configuration space is not isomorphic to $\RealNum^{\dimConfigSpace}$ it is only local due to the requirement of minimal coordinates $\genCoord(t)\in\RealNum^{\dimConfigSpace}$.
...
This chapter motivates three approaches for designing a closed-loop for fully actuated systems which are each motivated by the underlying rigid body structure.
