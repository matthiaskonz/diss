...
\end{align} 
\end{subequations}
The total energy $\totalEnergyC = \tfrac{1}{2} \sysVel^\top \sysInertiaMatC \sysVel + \potentialEnergyC$ is a Lyapunov function if the system inertia matrix $\sysInertiaMatC$, the system dissipation matrix $\sysDissMatC$ and the potential energy $\potentialEnergyC$ are positive definite. 
...
\end{alignat} 
\end{subequations}
This demonstrates the structural similarities among inertia, damping and stiffness.
Furthermore, the first order approximation can be insightful for parameter tuning purposes.
