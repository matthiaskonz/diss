...
\end{align} 
\end{subequations}
The total energy $\totalEnergyC = \tfrac{1}{2} \sysVel^\top \sysInertiaMatC \sysVel + \potentialEnergyC$ is a Lyapunov function if the system inertia matrix $\sysInertiaMatC$ and the potential energy $\potentialEnergyC$ are positive definite, and the system dissipation matrix $\sysDissMatC$ is positive semi-definite. 

\paragraph{Linearization.}
Assume that the configuration of the system is close to its reference, i.e.\ $\sysCoord \approx \sysCoordR$.
...
\end{alignat} 
\end{subequations}
This not shows the similarities between inertia, damping and stiffness.
Furthermore, the first order approximation can be very insightful for parameter tuning purposes.
