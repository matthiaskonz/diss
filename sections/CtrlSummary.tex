...
Let the rigid body system be parameterized by a set of configurations $\{\bodyHomoCoord{\BidxI}{\BidxII}\}$ formulated in terms of the chosen coordinates $\sysCoord$ as discussed in \autoref{sec:RBSParameterization}.
Let changes in the configuration coordinates be captured by the system velocity $\sysVel$ such that $\sysCoordd = \kinMat \sysVel$ as discussed in \autoref{sec:secMathMinVelCoord}.
Given these ingredients, we may use the rules from \autoref{sec:RBSParameterization} to compute any absolute or relative configurations $\{\bodyHomoCoord{\BidxI}{\BidxII}\}$ and the corresponding body Jacobians $\{\bodyJac{\BidxI}{\BidxII}\}$.
...
Only the energy-based approach additionally requires a transport map $\sysTransportMap$.
...
For the underactuated case ($\rank\sysInputMat<\dimConfigSpace$) the control input is computed such that the resulting system acceleration $\sysVeld$ deviates least from the desired acceleration $-\sysInertiaMatC^{-1}\sysForceC$ in the least squares sense with weights $\sysInertiaMatC$.
For a given left complement $\sysInputMatLComp$ we may also compute the matching force $\tuple{\lambda}$ which gives insight into how to adjust the control parameters $\{\bodyInertiaMatC{\BidxI}{\BidxII}, \bodyDissMatC{\BidxI}{\BidxII}, \bodyStiffMatC{\BidxI}{\BidxII}\}$ to minimize or even eliminate this deflection.
