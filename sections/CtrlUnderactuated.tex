...
With this, we may rewrite the residual Gaussian constraint from \eqref{eq:GaussianUnderactuated} as $\GaussianConstraintC_0 = \tfrac{1}{2} \norm[\sysInertiaMatC^{-1}]{\tilde{\sysForce}}^2$.
...
Furthermore, for this case, we have $\mat{P}^\bot = \mat{0}$ and consequently $\GaussianConstraintC_0=0$, $\tilde{\sysForce}=\tuple{0}$.

\subsection{Matching condition}\label{sec:MatchingCondition}
When solving \eqref{eq:MinProbUnderactuated}, instead of eliminating the accelerations $\sysVeld$, we may also eliminate the controls $\sysInput$:
...
However, the examples for which this approach is demonstrated restrict to stabilization tasks $\sysVelR = \tuple{0}$ for small academic systems.
...
The matching force \eqref{eq:MatchingCondition} may become very cumbersome for complex systems and might even be impossible to eliminate with the given parameters.
It may be instructive to analyze it for particular situations.
...
Assume that we have a small error $\LinErrorCoord = \kinMat^+(\sysCoordR)(\sysCoord-\sysCoordR)$ to a constant reference $\sysVelR=\tuple{0}$ as already considered in \autoref{sec:CtrlLinearization}.
...
In most control systems the control inputs $\sysInput$ cannot take arbitrary values, but due to practical limitations are required to be, for example, $\sysInputCoeff{a} \in [-\sysInputCoeff{a}^{\idxMax}, \sysInputCoeff{a}^{\idxMax}], a=1,\ldots,\numInputs$.
