...
\paragraph{Modeling.}
As the title suggests, this work deals with the modeling and control of rigid body systems.
The procedure for physical modeling is illustrated in \autoref{fig:ModelingIllustration}.
It starts by approximating the system under consideration with a mechanical model.
...
This work is concerned with this second part, i.e., the derivation of \textit{equations of motion}.
...
\end{align}
The equations of motion are derived from the Lagrangian $\Lagrangian$ as follows:
...
The goal is for the resulting controlled system $\dot{\sysState} = \tuple{f}(\sysState,\tuple{g}(\sysState,\tuple{r})) = \bar{\tuple{f}}(\sysState,\tuple{r})$ to satisfy design objectives.
...
From a mechanical point of view, these multicopters may be modeled as a free rigid body moving within Earth's gravity.
The difference between them is only in the placement of the actuators:
...
For the previous example of aerobatic motions it is evident that singularities of this form would be unacceptable for simulation and control design. 
...
Moreover, its mathematical structure reflects the physical symmetries of the model.
...
The Euler angles are unsuited at the so called gimbal lock ($\pit=\pm 90^\circ$) just as longitude fails at Earth's poles.
