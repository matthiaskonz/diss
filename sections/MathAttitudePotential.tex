...
In the following, we extend these results: we do not impose assumptions on $\mat{P}$, and we are also interested in other extrema of $\potentialEnergy$.
...
\end{align} 
\end{subequations}
This means flipping the direction of the singular vectors corresponding to the smallest singular value $\sigma_3$ if either $\mat{X}$ or $\mat{Y}$ contains a reflection.
...
\end{align}
For a critical point $\bar{\R}_0$ (i.e., $\differential \bar{\potentialEnergy}(\bar{\R}_0)=\tuple{0}$), the matrix $\bar{\mat{\varSigma}} \bar{\R}_0$ must be symmetric.
...
\qquad\Leftrightarrow \qquad
 ...
 \end{matrix}\right.
\end{align} 
It is crucial to notice that $\sigma_1 \geq \sigma_2 \geq |\bar{\sigma}_3| \geq 0$ implies $\lambda_3 \geq \lambda_2 \geq \lambda_1 \geq 0$.
...
The matrix $\R$ is unique if and only if $\mat{K}$ is positive definite.
