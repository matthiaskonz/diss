...
Since $\configSpace \subset \RealNum^{\numCoord}$, their composition $\potentialEnergy \circ \sysCoord = f : \RealNum \rightarrow \RealNum$ is a scalar function and has the Taylor expansion
\begin{multline}
...
\end{align}
\end{subequations}
Similarly, with $\differential = [\dirDiff{1}, \ldots ,\dirDiff{\dimConfigSpace}]^\top$ we have the matrix notation:
\begin{subequations}
\begin{align}
...
Unfortunately, this is (in general) not the case for directional derivatives like $\dirDiff{\LidxI}$ defined in \eqref{eq:DefBasisDiff}.
...
\nonumber\\
 ...
 .
\end{align}
As this relation holds for any function $\potentialEnergy$, we can state it in operator form and introduce the \textit{commutation coefficients} $\BoltzSym{\LidxIII}{\LidxI}{\LidxII}$ as
...
The case of redundant configuration coordinates and consequently a non-square matrix $\kinMat$ as derived above, is not established in the literature to the best of the author's knowledge.

It is worth noting that the commutation coefficients are \textit{invariant} under the choice of configuration coordinates $\sysCoord$, even though the coordinates appear explicitly in the definition:
...
This might be obvious from a geometric point of view, but can also be checked by explicit calculation.
It will even turn out that for most of our examples (like the one above) the coefficients will be constants.
...
&\Rightarrow&
 ...
 = \pdiff[\kinBasisMatCoeff{\LidxI}{\GidxII}]{\sysCoordCoeff{\GidxI}},&
&\Rightarrow&
...
\end{align}
Since this constraint is affine w.r.t.\ $\sysCoord$, it is reasonable to use a basis $\LinErrorCoord(t) \in \RealNum^{\dimConfigSpace}$ for the deviated configuration coordinates:
\begin{align}
...
\end{align*}
Evidently, the expressions simplify significantly if the velocity coordinates are integrable, i.e. $\BoltzSym{}{}{} = 0$, or if the approximation is about a static point $\sysCoordB = \const \Rightarrow \sysVel = \tuple{0}$.


\subsection{Calculus of variations}\label{sec:CalculusOfVariations}
The calculus of variations is concerned with the extrema of functionals, i.e., functions of functions. %\cite[sec.\,12]{Arnold:MathematicalMethodsOfClassicalMechanics}.
...
The \textit{Lagrangian} $\Lagrangian$ may be any sufficiently smooth function of the configuration coordinates $\sysCoord$, its derivatives $\sysCoordd = \kinMat \sysVel$ parameterized in the velocity coordinates $\sysVel$ and may depend explicitly on the time $t$ as well.

For the standard case, $\sysCoord = \genCoord$ and $\sysVel = \genCoordd$, a derivation may be found in e.g. \cite[chap.\,4, §3]{CourantHilbert1}, \cite[ch.\,II]{Lanczos:Variational} or \cite[sec.\,12]{Arnold:MathematicalMethodsOfClassicalMechanics}.
...
Assuming $\varParam$ to be sufficiently small, we may use the first order approximation analogous to \autoref{sec:LinAboutTraj}:
...
\end{align}
Now if $\sysCoord(t)$ is indeed the solution to the variational problem, then $\mathcal{P}(\varParam)$ must be stationary at $\mathcal{P}(0)$ and consequently $\spdiff[\mathcal{P}]{\varParam}(0) = 0$.
...
The time derivative of the Hamiltonian along the solutions of \eqref{eq:HamiltonsCanonicalEquations} is
...
\begin{Example}
Consider the Lagrangian $\Lagrangian = \tfrac{1}{2} \w^\top \bodyMOI{}{} \w$ from above with the velocity coordinates $\sysVel = \w$.
The corresponding generalized momentum and Hamiltonian are
\begin{align}
 \genMomentum &= \pdiff[\Lagrangian]{\w} = \bodyMOI{}{} \w
\qquad \Leftrightarrow \qquad 
 \w = \bodyMOI{}{}^{-1} \genMomentum
\\
 \Hamiltonian &= \tfrac{1}{2} \genMomentum^\top \bodyMOI{}{}^{-1} \genMomentum.
\end{align}
Evaluation of \eqref{eq:HamiltonsCanonicalEquations} yields
\begin{subequations}
\begin{align}
 \sysCoordd &= \kinMat \bodyMOI{}{}^{-1} \genMomentum
\\
 \dot{\genMomentum} &= \wedOp(\genMomentum) \bodyMOI{}{}^{-1} \genMomentum
\end{align}
\end{subequations}
As the Hamiltonian has no explicit time dependency, it must be constant along the solutions.
This may be directly checked from
\begin{align}
 \dot{\Hamiltonian} &= \genMomentum^\top \bodyMOI{}{}^{-1} \dot{\genMomentum}
 = \genMomentum^\top \bodyMOI{}{}^{-1} \wedOp(\genMomentum) \bodyMOI{}{}^{-1} \genMomentum
 = \tuple{0}.
\end{align}
