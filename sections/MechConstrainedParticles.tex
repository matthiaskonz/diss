...
\subsection{First principles}\label{sec:MechConstrainedFirstPrinciples}
...
The remarkable quality of Hamilton's principle is that analogous principles have since been found for all branches of physics:
...
Apart from physics, there is optimal control that builds up on essentially the same idea, see e.g.\ \cite{Bryson:AppliedOptimalControl}.
...
To address this one finds a similar statement in e.g.\ \cite[eq.\,12.2.14]{Lurie:AnalyticalMechanics} or \cite[sec.\ I.3]{Szabo:HM}:
...
\subsection{Coordinates}\label{sec:MechConstrainedCoordinates}
...
As motivated in \autoref{sec:MathCoordinates}, in some cases it can be beneficial to use a slightly larger number of \textit{redundant} generalized coordinates $\sysCoord(t) \in \configSpace = \{ \sysCoord \in \RealNum^{\numCoord} \, | \, \geoConstraint(\sysCoord) = \tuple{0} \}$ and \textit{minimal} velocity coordinates $\sysVel(t) \in \RealNum^{\dimConfigSpace}$ related by $\sysCoordd = \kinMat \sysVel$.
...
Analogous to the velocity, we may parameterize virtual displacements as $\delta \particlePos{\PidxI} = \dirDiff{\LidxI} \particlePos{\PidxI} \varCoordCoeff{\LidxI}, \PidxI = 1,\ldots,\numParticles$ in terms of \textit{minimal} displacement coordinates $\varCoord\in\RealNum^\dimConfigSpace$.
...
It is worth noting that the quantity $2\kineticEnergy$ which appeared in this context, was called \textit{vis viva} in older publications and translated to \textit{living force} or \textit{lebendige Kraft}, e.g.\ \cite{Hamel:LagrangeEuler} or \cite[chap.\ 5]{Klein:EntwicklungDerMathematik}.
...
\begin{align}
 ...
 \nonumber\\
  ...
  \LidxI = 1,\ldots,\dimConfigSpace.
\end{align}
The formulations with $\kineticEnergyInertial$ and $\accEnergyInertial$ are not really important for the modeling case here.
However, they do give some physical insight: 
Instead of working with ``absolute'' energies formulated in terms of ``absolute'' acceleration $\particlePosdd{\PidxI}$ and velocity $\particlePosd{\PidxI}$, the quantities $\kineticEnergyInertial$ and $\accEnergyInertial$ are metrics for the deviation of the system from its natural motion, i.e.\ the free fall.
This is very much the spirit of general relativity and Gauss's principle.
Furthermore, this will be also be a core concept for the control design in the next chapter where the reference motion will replace the free fall as natural motion.
...
Then, as proposed in \eqref{eq:ParticleDamping}, each particle is subject to a friction force $\particleForceDiss{\PidxI} = \particleDamping{\PidxI} (\fluidVel{\PidxI} - \particlePosd{\PidxI})$.
...
\begin{align}
 ...
 = \sumParticles \particleDamping{\PidxI} \sProd{\pdiff[\particlePosd{\PidxI}]{\sysVelCoeff{\LidxI}}}{\fluidVel{\PidxI} - \particlePosd{\PidxI}}
...
Note that non-negativity of the damping parameters $\particleStiffness{} \geq 0$ implies non-negativity of the dissipation function $\dissFkt \geq 0$.
...
\\
 ...
 .
\end{align}
Evidently, for a scleronomic system, the Hamiltonian $\Hamiltonian$ coincides with the total energy $\totalEnergy$. 
