...
So far we only assumed only geometric constraints, and essentially eliminated them by suitable choice of velocity coordinates.
In this section we consider additional, more general constraints that may also depend on velocity and/or even acceleration, but sticks to the chosen coordinates $\sysCoord$ and $\sysVel$.
...
Then Gauss' principle states that the equations of motion for the system with the additional constraint states must solve the following minimization problem
...
Similar to the choice of minimal velocity coordinates in \autoref{sec:MathCoordinates}, we may formulate \textit{all} solution to the constraint equation as $\sysVeld = \mat{W}\tuple{\alpha} + \tuple{w}$.
...
These equations of motion differ from the the ones above in the nonholonomic case, i.e. unless the constraints are \textit{integrable}.
...
On the other side, there are no real world examples of nonlinear nonholonomic constraints \cite[p.\ 499]{Hamel:TheoretischeMechanik}, \cite[ch.\ IV]{Neimark:NonholonomicSystems}, \cite[p.\ 96]{Schwertassek:MultibodySystems} and none of acceleration constraints \cite[p.\ 505 \& 525]{Hamel:TheoretischeMechanik}, so there is no way of \textit{physically} validate the resulting equations of motion.
...
Gauss suggested in \cite{Gauss:Principle} also the application to inequality constraints which will not be discussed here.
