...
So far we have only assumed geometric constraints, and essentially eliminated them by suitable choice of velocity coordinates.
In this section we consider additional, more general constraints that may also depend on velocity and/or even acceleration, but stick to the chosen coordinates $\sysCoord$ and $\sysVel$.

\paragraph{Types of constraints.}
Recall from \eqref{eq:DiffConstraint} that any smooth constraint is equivalent to its derivative supplemented by the appropriate initial condition.
...
Then Gauss' principle states that the equations of motion for the system with the additional constraints must solve the following minimization problem
...
Let $\rank\accConstraintMat = c \leq \dimConfigSpace$ and assume that $\accConstraintB$ lies in the column space $\accConstraintMat$, i.e. the constraint equation is solvable.
For $c=\dimConfigSpace$ the trivial solution is $\sysVeld = \accConstraintMat^{-1}\accConstraintB$.
For the common case $0 < c < \dimConfigSpace$ we may consider two solution approaches:

\paragraph{Solution using null space.}
Similar to the choice of minimal velocity coordinates in \autoref{sec:MathCoordinates}, we may formulate \textit{all} solutions to the constraint equation as $\sysVeld = \mat{W}\tuple{\alpha} + \tuple{w}$.
...
Another common solution approach to \eqref{eq:GaussPrincipleAccConstraints} utilizes the concept of \textit{Lagrange multipliers} (see e.g.\ \cite[ch.\,14]{Luenberger:LinearAndNonlinearProgramming}):
...
\end{align}
Similar to \autoref{sec:MechConstrainedFirstPrinciples} on the principle of constraint release, the term $\accConstraintMat^\top \LagrangeMult$ may be interpreted as the generalized reaction force on the system to enforce the constraint.
...
Restricting to geometric constraints and linear velocity constraints, we could derive the same results using d'Alembert's principle \eqref{eq:DAlembertPrinciple}.
Applying Hamilton's principle \eqref{eq:HamiltonPrinciple} to a system with velocity constraints, i.e.\ constraining the minimizing path by the constraints leads to so-called vakonomic mechanics, a term coined in \cite{Kozlov:vakonomic}.
These equations of motion differ from the ones above in the nonholonomic case, i.e. unless the constraints are \textit{integrable}.
This issue may be mended by imposing the kinematic constraint on the variation, see \cite[pp.\ 208--213]{Bloch:NonholonomicMechanics}.

From the first principles stated in \autoref{sec:MechConstrainedFirstPrinciples}, Gauss' principle is the only one that is applicable for the case of acceleration constraints.
The possibility of handling such a variety of constraints might put Gauss' principle in a superior position compared to other principles as pointed out in \cite[p.\ 525]{Hamel:TheoretischeMechanik}.
On the other hand, there are no real-world examples of nonlinear nonholonomic constraints \cite[p.\ 499]{Hamel:TheoretischeMechanik}, \cite[ch.\ IV]{Neimark:NonholonomicSystems}, \cite[p.\ 96]{Schwertassek:MultibodySystems} and none of acceleration constraints \cite[p.\ 505 \& 525]{Hamel:TheoretischeMechanik}, so there is no way of \textit{physically} validating the resulting equations of motion.
...
Gauss also suggested in \cite{Gauss:Principle} the application to inequality constraints, which will not be discussed here.
