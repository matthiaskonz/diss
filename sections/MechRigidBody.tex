...
Established textbooks on physics (e.g.\ \cite[chap. 4]{Goldstein:ClassicalMechanics}, \cite[§31]{Landau:Mechanics} or \cite[§44]{Boltzmann:PrincipeDerMechanik}) define a \textit{rigid body} as a system of a \textit{finite} number $\numParticles$ particles such that the distances $d_{\PidxI\PidxII} = \norm{\particlePos{\PidxI} - \particlePos{\PidxII}}$ between their positions $\particlePos{\PidxI}$ are constant.
...
In particular, for the latter two parts, the model of concentrated particles is more intuitive in the author's humble opinion.
...
In order to comply with the framework from the previous chapter we may group the configuration coordinates as $\sysCoord = [\r^\top, \Rx^\top, \Ry^\top, \Rz^\top]^\top \in \configSpace \subset\RealNum^{12}$ and the geometric constraints as $\geoConstraint(\sysCoord) = \tuple{0}$ are the constraints of $\SpecialOrthogonalGroup(3)$ as given in \eqref{eq:constraintSO3}.
...
These will now be applied to the rigid body with the chosen coordinates.
...
Plugging the kinetic energy \eqref{eq:RigidBodyKineticEnergy} into the corresponding formulation \eqref{eq:InertiaForceKinEnergy} of the generalized inertia force and using the commutation symbols from \eqref{eq:RBCommutationCoeffAd} yields
...
\end{figure}
Assume that every particle of the rigid body with position $\particlePos{\PidxI}$ is connected to a position $\springHubPos{\PidxI} \in \RealNum^3$ by a linear spring with stiffness $\particleStiffness{\PidxI} \in \RealNumSP$, see \autoref{fig:RBStiffness}.
...
Consequently, any constellation may be captured by the $1+3+3+9=16$ parameters within $(\bodyStiffness{}{}, \bodyCOS{}{}, \tuple{p}, \mat{P})$ and some constant offset.
...
\begin{align}\label{eq:RBStiffnessPotentialDifferential}
\differential \potentialStiff(\sysCoordR)
 = \begin{bmatrix} \bodyStiffness{}{} (\RR^\top (\rR - \tuple{p}) + \bodyCOS{}{}) \\ \bodyStiffness{}{} \wedOp(\bodyCOS{}{}) \RR^\top \rR + \veeTwoOp(\mat{P} \RR) \end{bmatrix} = \tuple{0}
 .
\end{align}
...
\end{align}
The solution to this subproblem $\veeTwoOp(\mat{P}_{\!s} \RR) = \tuple{0}, \RR\in\SpecialOrthogonalGroup(3)$ was discussed in great detail in \autoref{sec:AppendixAttitudePotential}:
...
Then $\sysCoordR \cong (\rR, \RR) = (\tuple{p} - \mat{X} \bodyCOS{}{}, \mat{X})$ is a critical point.
...
We may use the results above to substitute $\tuple{p}$ and $\mat{P}$ by the minimum configuration $(\rR, \RR)$ and the matrix $\bodyMOSs{}{}$.
With a rather lengthy calculation we may reformulate the potential form \eqref{eq:RBStiffnessPotential2} as
\begin{subequations}\label{eq:RBStiffnessPotential3}
\begin{align}
 \potentialStiff(\sysCoord) &= \tfrac{1}{2} \bodyStiffness{}{}\norm{\r\!+\!\R\bodyCOS{}{} - (\rR \!+\! \RR\bodyCOS{}{})}^2
...
%   - \tr\big((\wedMatOp(\bodyMOSs{}{}) \RR^\top \!+\! \bodyStiffness{}{}\bodyCOS{}{}(\rR \!+\! \RR \bodyCOS{}{})^\top) \R\big)
% \nonumber\\
%  &\qquad + \tfrac{1}{2} \sumParticles \particleStiffness{\PidxI} \big( \norm{\particleBodyPos{\PidxI}}^2 + \norm{\springHubPos{\PidxI}}^2)
% \nonumber\\
%  &= \tfrac{1}{2} \bodyStiffness{}{}\norm{\r + \R\bodyCOS{}{} - (\rR + \RR\bodyCOS{}{})}^2
%   + \tr\big(\wedMatOp(\bodyMOSs{}{})(\idMat[3]-\RR^\top \R)\big)
% \nonumber\\
%  &\qquad - \tfrac{1}{2} \bodyStiffness{}{}\norm{\rR}^2 - \bodyStiffness{}{}\sProd{\rR}{\RR\bodyCOS{}{}} - \bodyStiffness{}{}\norm{\bodyCOS{}{}}^2 - \tr(\wedMatOp(\bodyMOSs{}{})) + \tfrac{1}{2} \sumParticles \particleStiffness{\PidxI} \big( \norm{\particleBodyPos{\PidxI}}^2 \!+\! \norm{\springHubPos{\PidxI}}^2)
% \nonumber\\
%  &= \tfrac{1}{2} \bodyStiffness{}{}\norm{\r + \R\bodyCOS{}{} - (\rR + \RR\bodyCOS{}{})}^2
%   + \tr\big(\wedMatOp(\bodyMOSs{}{})(\idMat[3]-\RR^\top \R)\big)
% \nonumber\\[-2ex]
...
%     - \tr\big(\mat{P} \RR\big)
%      + \tfrac{1}{2} \sumParticles \particleStiffness{\PidxI} \big( \norm{\particleBodyPos{\PidxI}}^2 \!+\! \norm{\springHubPos{\PidxI}}^2)
%   + \bodyStiffness{}{} \sProd{\rR}{\overbrace{\rR + \RR\bodyCOS{}{} - \tuple{p}}^{\tuple{0}}}
% \nonumber\\
%  &= \tfrac{1}{2} \bodyStiffness{}{}\norm{\r + \R\bodyCOS{}{} - (\rR + \RR\bodyCOS{}{})}^2
%   + \tr\big(\wedMatOp(\bodyMOSs{}{})(\idMat[3]-\RR^\top \R)\big)
% \nonumber\\
%  &\qquad+ \tfrac{1}{2} \sumParticles \particleStiffness{\PidxI} \big( \norm{\particleBodyPos{\PidxI}}^2 + \norm{\springHubPos{\PidxI}}^2 + \norm{\rR}^2 - 2\sProd{\RR\particleBodyPos{\PidxI}}{\springHubPos{\PidxI}} + 2\sProd{\rR}{\RR\particleBodyPos{\PidxI} - \springHubPos{}})
% \nonumber\\[1ex]
%  &= \tfrac{1}{2} \bodyStiffness{}{}\norm{\r + \R\bodyCOS{}{} - (\rR + \RR\bodyCOS{}{})}^2
%   + \tr\big(\wedMatOp(\bodyMOSs{}{})(\idMat[3]-\RR^\top \R)\big)
% \nonumber\\
%  &\qquad+ \underbrace{\tfrac{1}{2} \sumParticles \particleStiffness{\PidxI} \norm{\rR + \RR\particleBodyPos{\PidxI} - \springHubPos{}}^2}_{\potentialStiff(\sysCoordR) \, = \, \potentialStiff_0 \, = \, \const}.
% \end{align}
where $\bodyMOS{}{} = \bodyMOSs{}{} + \bodyStiffness{}{} \wedOp(\bodyCOS{}{}) \wedOp(\bodyCOS{}{})^\top$.
The first parts vanish at the minimal configuration $\r=\rR$, $\R=\RR$ and the constant $\potentialStiff_0 = \potentialStiff(\sysCoordR) = \tfrac{1}{2} \sumParticles \particleStiffness{\PidxI} \norm{\rR + \RR\particleBodyPos{\PidxI} - \springHubPos{}}^2$ is the potential of the residual displacement of the springs at this configuration.
The differential may be written as
\begin{align}\label{eq:RBStiffnessForce}
 \differential\potentialStiff(\sysCoord) = 
 \begin{bmatrix} 
   \bodyStiffness{}{} \R^\top (\r + \R\bodyCOS{}{} - (\rR + \RR\bodyCOS{}{})) \\
   \bodyStiffness{}{}\wedOp(\bodyCOS{}{}) \R^\top (\r-\rR) + \veeTwoOp\big(\wedMatOp(\bodyMOS{}{}) \RR^\top\R\big)
 \end{bmatrix}.
\end{align}
...
\item The matrices $\bodyInertiaMatp{}{}, \bodyDissMatp{}{}, \bodyStiffMatp{}{} \in \SymMatSP(4)$ capture the distribution of mass, damping and stiffness.
