...
As motivated in the single rigid body case, let there be a body fixed frame for each body of the system as illustrated in \autoref{fig:RBSConfigurations}.
...
The configuration can also be expressed w.r.t.\ any other body: $\bodyPos{\BidxI}{\BidxII}$ is the position of the $\BidxII\text{-th}$ frame w.r.t.\ the frame of the $\BidxI\text{-th}$ body and analog for the attitude $\bodyRot{\BidxI}{\BidxII}$.
...
\begin{align}
 ...
 \begin{bmatrix}
  ...
  \bodyRotzx{1}{2} & \bodyRotzy{1}{2} & \bodyRotzz{1}{2} & \LoadHeight \\
  0 & 0 & 0 & 1
 \end{bmatrix}.
\end{align}
...
\end{align}
\end{subequations}
The set of configurations $\HomoCoordGraph_0 = \{ \bodyHomoCoord{0}{1}, \bodyHomoCoord{1}{2}, \bodyHomoCoord{1}{3}, \bodyHomoCoord{3}{4}, \bodyHomoCoord{1}{5}, \bodyHomoCoord{5}{6}, \bodyHomoCoord{1}{7}, \bodyHomoCoord{7}{8} \}$ forms a directed graph as shown at the bottom of \autoref{fig:TriV3ConfigGraph}.
...
This example was mainly chosen because the Tricopter will be discussed in the following chapters.
However, it is also complex enough that one probably does not want to derive the equations of motion without a formalism.
...
If use the notation $\bodyCoord{\BidxI}{\BidxII}(\sysCoord)$ and $\bodyKinMat{\BidxI}{\BidxII}(\sysCoord)$ for the body coordinates in tuple form as defined in \eqref{eq:RBKinematicEq2}, then we may also express the body Jacobian as
\begin{align}
 \bodyJac{\BidxI}{\BidxII} = \bodyKinMat{\BidxI}{\BidxII}^+ \pdiff[\bodyCoord{\BidxI}{\BidxII}]{\sysCoord} \kinMat
 .
\end{align}
...
\end{align}
\end{subequations}
From this it should be clear how the corresponding body Jacobians look, e.g.\ $\bodyJac{0}{1} = [\idMat[6] \ 0]$.
...
Combining this with the kinetic energy \eqref{eq:RigidBodyKineticEnergy} of a single free rigid body and the formulation of the absolute body velocities $\bodyVel{0}{\BidxII}$ in terms of the chosen coordinates using the body Jacobian $\bodyJac{0}{\BidxII}(\sysCoord)$ from \eqref{eq:DefBodyJacobian}, yields
...
With a quite cumbersome computation that is not stated here, one may show that the connection coefficients $\ConnCoeffL{\LidxI}{\LidxII}{\LidxIII}$ associated to the system inertia matrix $\sysInertiaMat$ from \eqref{eq:RBSKineticEnergy} and the commutation coefficients $\BoltzSym{\LidxI}{\LidxII}{\LidxIII}$ of the chosen parmeterisation, can also be expressed as
\begin{subequations}\label{eq:RBSConnCoeff}
\begin{align}
...
\\
 &\ \ \vdots \nonumber
\\
 &= \sumBodies \bodyJacCoeff{0}{\BidxII}{\LBidxI}{\LidxI} \big( \bodyInertiaMatCoeff{0}{\BidxII}{\LBidxI\LBidxII} \dirDiff{\LidxIII} \bodyJacCoeff{0}{\BidxII}{\LBidxII}{\LidxII} + \tfrac{1}{2} \big( \bodyBoltzSym{\LBidxIV}{\LBidxI}{\LBidxII} \bodyInertiaMatCoeff{0}{\BidxII}{\LBidxIV\LBidxIII} + \bodyBoltzSym{\LBidxIV}{\LBidxI}{\LBidxIII} \bodyInertiaMatCoeff{0}{\BidxII}{\LBidxIV\LBidxII} - \bodyBoltzSym{\LBidxIV}{\LBidxII}{\LBidxIII} \bodyInertiaMatCoeff{0}{\BidxII}{\LBidxIV\LBidxI} \big) \bodyJacCoeff{0}{\BidxII}{\LBidxII}{\LidxII} \bodyJacCoeff{0}{\BidxII}{\LBidxIII}{\LidxIII} \big)
\\
 &= \sumBodies \bodyJacCoeff{0}{\BidxII}{\LBidxI}{\LidxI} \big( \bodyInertiaMatCoeff{0}{\BidxII}{\LBidxI\LBidxII} \dirDiff{\LidxIII} \bodyJacCoeff{0}{\BidxII}{\LBidxII}{\LidxII} + \bodyConnCoeffL{0}{\BidxII}{\LBidxI}{\LBidxII}{\LBidxIII} \bodyJacCoeff{0}{\BidxII}{\LBidxII}{\LidxII} \bodyJacCoeff{0}{\BidxII}{\LBidxIII}{\LidxIII} \big)
 \label{eq:RBSConnCoeffL3}
\end{align}
\end{subequations}
in terms of the body inertia $\bodyInertiaMat{0}{\BidxII}$, the body Jacobians $\bodyJac{0}{\BidxII}$, the body commutation coefficients $\bodyBoltzSym{}{}{}$ (see \eqref{eq:RBCommutationCoeff}) or the the body connection coefficients $\bodyConnCoeffL{0}{\BidxII}{}{}{}$ (see \eqref{eq:RBConnCoeffRes}).
As $\bodyInertiaMat{0}{\BidxII}$, $\bodyBoltzSym{}{}{}$, $\bodyConnCoeffL{0}{\BidxII}{}{}{}$ are constants, this shows again that the body Jacobians $\bodyJac{0}{\BidxII}$ completely capture the geometric information about the system.

The individual sum terms in \eqref{eq:RBSConnCoeffL3} resemble the basis transformation rule for connection coefficients (see e.g.\ \cite[Vol.\,2, p.\,221]{Spivak:DiffGeo} or \cite[p.\,145]{Abraham:FoundationsOfMechanics} for the special case of Christoffel symbols).
Loosly speaking: If we regard $\bodyJac{0}{\BidxII}^\top$ as the projection from $\Tangent[\bodyHomoCoord{0}{\BidxI}(\sysCoord)] \SpecialEuclideanGroup(3)$ to $\Tangent[\sysCoord]\configSpace$, then \eqref{eq:RBSConnCoeffL3} resembles the sum of the projections of the body connection coefficients $\bodyConnCoeffL{0}{\BidxII}{}{}{}$.
We have seen this above for the inertia matrix $\sysInertiaMat$ in \eqref{eq:RBSKineticEnergy}, and will see it for the inertial forces $\genForceInertia$ in \eqref{eq:RBSInertiaForceProj}.
...
\end{align}
For interpretation, we may rearange this to
\begin{align}\label{eq:RBSInertiaForceProj}
 \genForceInertia = \sumBodies \bodyJac{0}{\BidxII}^\top \underbrace{\Big( \bodyInertiaMat{0}{\BidxII} \ddt(\bodyJac{0}{\BidxII} \sysVel) - \ad{\bodyJac{0}{\BidxII}\sysVel}^\top \bodyInertiaMat{0}{\BidxII}\bodyJac{0}{\BidxII} \sysVel \Big)}_{\bodyGenForceInertia{0}{\BidxII}}
 .
\end{align}
So the generalized inertia force $\genForceInertia$ of the rigid body system can be regarded as the sum of the projections of the individual body inertia forces $\bodyGenForceInertia{0}{\BidxII}$, see \eqref{eq:InertiaForceRB}, using the body Jacobians $\bodyJac{0}{\BidxII}$.
This resembles the original definition \eqref{eq:ConstraintReleaseEliminated} of the generalized inertia force $\genForceInertiaCoeff{\LidxI} = \sumParticles \sProd{\dirDiff{\LidxI} \particlePos{\PidxI}}{\particleMass{\PidxI} \particlePosdd{\PidxI}}$ as the sum over the projections of the inertial force of the individual particles.

Similar results \eqref{eq:RBSInertiaForce} are called \textit{the projection equation} in \cite[sec.\ 4.2.5]{Bremer:ElasticMultibodyDynamics} and \textit{the Kane equations} \cite[chap.\ 6]{Kane:Dynamics}.
...
The generalized force of gravity on a rigid body system may be formulated as
...
Finally, the generalized force due to an arbitrary constellation of linear springs on a rigid body \textit{system} may be formulated as
...
\end{multline}
Note that we obtain the same results for a time variant $\bodyHomoCoordR{\BidxI}{\BidxII}(t)\in \SpecialEuclideanGroup(3)$.
Only the derivation is slightly more cumbersome.
...
For a system of rigid bodies we have the dissipation function
\begin{align}
...
The remaining steps only require basic linear algebra, so can be performed numerically.
