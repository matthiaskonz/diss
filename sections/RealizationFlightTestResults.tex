...
This section finally discusses results of actual flight tests.
...
The data for the presented figures were recorded using the body-based control approach.

The controller parameters for the following experiments are summarized in \autoref{tab:ParamTriQuadCtrl}.
Note that the values for inertia etc.\ are with respect to the body-fixed frame and not with respect to the (closed loop) center of mass as in \autoref{sec:CtrlExampleQuadcopter}.
Consequently, some values differ from what the matching conditions in \eqref{eq:CtrlExampleQuatMatchedParam} would suggest.
...
\begin{table}
 ...
 \begin{tabular}{crlrll}
  ...
  $\kapcxx=\kapcyy$ & $1.7$&$\tfrac{\unit{kg}\,\unit{m}^2}{\unit{s}^2}$& $2.6$&$\tfrac{\unit{kg}\,\unit{m}^2}{\unit{s}^2}$ & tilt stiffness \\
  $\kapczz$ & $0.7$&$\tfrac{\unit{kg}\,\unit{m}^2}{\unit{s}^2}$ & $0.2$&$\tfrac{\unit{kg}\,\unit{m}^2}{\unit{s}^2}$ & yaw stiffness \\
  \midrule
...
Firstly, the copters behave differently for the tilting degree of freedom and for heading.
This is also reflected by different controller parameters for the respective directions.
Secondly, for the quadcopter, tilting is used to compensate a horizontal position error.
...
In the first $10\,\unit{s}$ the tricopter performs horizontal movement while maintaining attitude and height.
For the next $10\,\unit{s}$ it tracks similar trajectories, but this time uses body tilt to generate the necessary horizontal force.
...
One can see that for a body tilt of $15^\circ$, the servos must tilt more than $30^\circ$.
The last transition is a $360^\circ$ heading rotation and $1\,\unit{m}$ height change in $2\,\unit{s}$.
This is where we observe the maximum attitude error of about $5^\circ$.
Here in particular, the heading error $e_{\head}$ is the major part of the attitude error $e_{\R}$.

Comparing the performance of this tricopter experiment with the following quadcopter experiments in \autoref{tab:MulticopterExperimentStatistics}, the tricopter has the lowest position and attitude error.
...
The reference trajectories are very similar to the previous transitions for the tricopter, but with reduced transition times.
...
This is the first maneuver that involves the quadcopter being upside down, i.e.\ $\RR \ez = -\ez$, so an appropriate method for heading reference generation is required.
Though the accelerations are quite high as well, this experiment has, compared to the others, the highest translational velocity.
The fact that it has the largest position and attitude errors could suggest that there are large model errors and/or unknown disturbances related to higher translational velocity.
...
The maximal linear acceleration of $17.5\,\tfrac{\unit{m}}{\unit{s}^2}$ occurs when the quadcopter is upside down.
...
Successful experimental results for aerobatic maneuvers with quadcopters have been shown in the literature:
In \cite{Lupashin:QuadrocopterMultiFlips}, a triple loop is achieved by open-loop control and iterative learning techniques.
In \cite{MellingerIJRR12}, case studies show tracking aggressive trajectories planned through iterative refinement and controlled by switching controllers for different trajectory segments.
In contrast, the results here were achieved with one controller and constant parameters.
