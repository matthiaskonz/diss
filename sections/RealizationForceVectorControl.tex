...
This force cannot be realized instantaneously, since it is subject to its own actuator dynamics; see \autoref{sec:RealizationModels}.
This section describes a control approach for the generalized force $\genForce$.

Because the force $\genForce$ is itself subject to actuator dynamics, it is not possible to realize it instantaneously.
However, we can aim to realize it reasonably fast, which is the subject of this section.
At the same time we need an accurate estimate of the current generalized force for the rigid body observer.
...
A dedicated test bench (see \autoref{fig:PropellerTestBench}) with an absolute encoder for the tilt angle $\aServo$ was used to validate the servo dynamics \eqref{eq:TriModelServoDynamics} and identify its parameters $\ServoParam[0]$ and $\ServoParam[1]$.
Since the model is asymptotically stable, a simple online simulation of the model with the known servo setpoint $\aServoU[][k]$ is sufficient to estimate the current servo angle.
...
% \end{align}
In \autoref{fig:ServoSimRes} the simulation result is compared to the encoder measurement.
To be close to the real application on the tricopter, the propeller on the test bench is spinning at about $70\,\unit{Hz}$ during the experiment.
This explains the vibration seen in the encoder measurements.
If the propeller is switched off, the estimation error remains within $\pm 0.5\,\unit{DEG}$.
...
The scaled model \eqref{eq:PropDynamics} also contains a friction parameter $\PropParam[3] = \tfrac{\BLDCFriction}{\PropInertia}$, which we estimate online for each propeller drive individually.
...
\end{align}
For the linearization about the expansion point $e = 0$, $\PropVel = \PropVelStat$, the corresponding characteristic polynomial has the root
\begin{align}
...
From the structure of $\GenForceTransferFunction$ in \eqref{eq:TriActuatorTransferFunction}, it is clear that its entries are linear combinations of the transfer functions of the thrust and the servos, $\ThrustTransferFunction$ and $\ServoTransferFunction$.
Also, if $\ThrustTransferFunction = \ServoTransferFunction$, then $\GenForceTransferFunction$ would be diagonal.
...
\autoref{fig:BodeTriActorDyn} shows an exemplary Bode magnitude plot of $\GenForceTransferFunctionNorm$ for the hover case $\aServoStat = [0,0,0]^\top$ (left) and for a forward force with $\aServoStat = [\sfrac{\pi}{4},0,-\sfrac{\pi}{4}]^\top$ (right).
