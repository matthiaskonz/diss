...
This driver implements the electronic commutation and a digital current control with a sampling frequency of $64\,\unit{kHz}$.
It receives the desired torque-generating current $\BLDCCurr$ from the main controller at a sampling frequency of $200\,\unit{Hz}$.
Consequently, we assume that all electrical dynamics of the propeller drive are negligible on the timescale of the main controller and that the motor torque on the propeller is proportional to its current as
\begin{align}
...
\end{align}
Comparison of the resulting model and actual measurements in \autoref{fig:ServoSimRes} shows that this assumption is valid here.

\paragraph{Propeller aerodynamics.}
Propellers are designed to generate a force in their spinning direction.
A general theory of propellers is quite sophisticated; see, e.g., \cite{Johnson:HelicopterTheory}.
However, for the small, rigid propellers used on the LSR quadcopter and tricopter, a simple model that is also common in the corresponding literature (e.g.\ \cite{Mahony:Quadrotor2002}, \cite{MellingerIJRR12}, \cite{Fritsch:Diss}) is sufficient:
...
This introduces eight additional constant model parameters.
Even though their model has a nice physical motivation, it does not fit the experimental observations made for this work.
It does, however, motivate linear drag forces and torques on the multicopter body.
...
Here we present a generic model for a multicopter with $\numProp$ tiltable propellers.
...
\paragraph{Assumptions on the mass distribution.}
...
Assume that the combined center of mass of one arm and its propeller lies on the tilt axis.
This implies that the overall center of mass $\bodyCOM{}{}{} \in \RealNum^{3}$ of the complete system is constant with respect to the body-fixed frame.
...
A crucial part of getting the overall dynamics right is to explicitly consider the rotor of the servo motor.
...
%  \PropDir[2] = \PropDir[3] = 1.
 \PropDir = [-1, +1, +1]^\top.
\end{align}
The model equations do not simplify much, and the special result is not displayed explicitly here.
...
Since the propellers do not tilt ($\PropTilt = 0$), the corresponding equations can be regarded as definitions of reaction torques $\ServoTorque$ and dropped from the model equations.
...
Some crucial parameters are identified in dedicated experimental setups, which are discussed below.
The remaining parameters are estimated by least squares based on the system model and actual in-flight measurements.
...
It is essentially one tricopter arm with an additional absolute encoder for the tilt angle $\PropTilt$ and an incremental encoder for the propeller velocity $\PropVel$.
...
\end{align}
The equations are normalized, yielding the parameters $\PropParam[1]$, $\PropParam[2]$, $\PropParam[3]$ and $\ServoParam[0]$, $\ServoParam[1]$, which are more convenient later.
These parameters can be directly estimated from measured time series using Savitzky-Golay derivative estimation and least squares.
...
The pole stands on a digital scale, and the ``weight'' was recorded at different propeller speeds $\PropVel$.
...
A dedicated experiment was conducted for identification of the inertia parameters $\Jxx$, $\Jyy$, and $\Jzz$ for the quadcopter and tricopter.
The body is suspended by two strings, as shown on the left side of \autoref{fig:QuadPendulumIdent}.
The basic idea is that a rotation of the body about the vertical axis results in an upward movement of the body, which is countered by gravity.
Thus we effectively have a torsional spring that, together with the inertia, forms an oscillator whose frequency is proportional to the moment of inertia.
...
\end{align}
Here $\m$ indicates the total mass of the copter, $\gravityAccConst = 9.81\,\sfrac{\unit{m}}{\unit{s}^2}$ is the gravitational acceleration, and $d$ and $h$ are the distance and length of the strings.
...
This test was conducted for all three axes, i.e.\ for $\Jxx$, $\Jyy$, and $\Jzz$ for tricopter and quadcopter, and the resulting parameters are given in \autoref{tab:ParamTriQuad}.
...
The model \eqref{eq:RealizationMulticopterKinetics} still contains several unknown parameters.
These include the coordinates of the center of mass $\bodyCOM{}{}{} = [\sx, \sy, \sz]^\top$, the off-diagonal inertia entries $\Jxy$, $\Jxz$, and $\Jyz$, the translational damping $\dvx$, $\dvz$, the rotational damping $\sigx$, $\sigz$, and the coordinate $\lz$ of the center of damping.

With some auxiliary transformations ($\lzp = \dvx\lz$, $\sigxp = \sigx + \dvz\lz^2$), the first six model equations \eqref{eq:RealizationMulticopterKinetics} are linear in the remaining unknown parameters.
Thus we can identify these by least squares from flight-test measurements.
...
From this experiment, we obtain the first estimate for the overall center of mass $\bodyCOM{}{}{}$ of the tricopter and quadcopter.
The identified value $\sz$ is then used to adjust the body-fixed frame in the vertical direction such that $\sz=0\,\unit{m}$.
For the tricopter, this also determines the parameter $\ArmHeight$.
The values for $\sx$ and $\sy$ are small so we stick to the geometric center in the horizontal directions.

The damping parameter values for the tricopter are greater than those for the quadcopter.
This could be explained by the model from \cite{Martin:AccelerometerFeedback} and by the fact that the propellers of the tricopter nominally spin $40\%$ faster than those of the quadcopter.
...
This means in particular that $\bodyCOM{}{}{} = 0$ and $\bodyMOI{}{}{} = \diag(\Jx, \Jy, \Jz)$.
...
Comparing the computed accelerations from the flight test in \autoref{fig:ValidationTricopter} yields the following observations.
For the translational $\vxd$, $\vzd$, and the propeller acceleration $\PropVeld[1]$, the two models cannot be distinguished and they match the measured accelerations quite well.
For the angular accelerations $\wxd$ and $\wzd$, the models differ slightly.
Unfortunately, one cannot say that the full model matches the measured accelerations better than the simplified model.
This implies that there are unmodeled effects with greater influence than the ones we dropped for the simplified model.
Overall, the simplified model \eqref{eq:TricopterSimpModel} captures the tricopter dynamics reasonably well and, due to its simplicity, is a good starting point for model-based control design.

\paragraph*{Quadcopter.}
As simplifications for the quadcopter model, we neglect the inertial couplings between the center body and propellers except $\PropInertia \mat{H}_1 \PropVeld$.
