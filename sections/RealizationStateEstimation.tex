...
The rigid body attitude may be parameterized by a unit quaternion $\quat = [\quatw, \quatx, \quaty, \quatz] \in \Sphere^3$, which allows for a more efficient implementation on the microcontroller and is common in this field.
...
While the attitude estimate is used for outdoor applications with the multicopter, it is not used here because in the lab the more precise estimate from the motion capturing system is available.
...
We call the remaining constant misalignment $\RIMUBR \in \SpecialOrthogonalGroup(3)$.
While the gyroscope bias is compensated, the accelerometer contains a noticeable slowly varying bias $\aIMUB \in \RealNum^3$.
Furthermore, both sensors contain noticeable noise $\wIMUN[k], \aIMUN[k] \in \RealNum^3$, which is not further investigated here.
...
In contrast to the IMU measurements, there is no evidence for systematic errors in the VICON measurements.
However, their processing and transmission to the multicopter processor results in substantial latency, i.e.\ measurements corresponding to time $t$ are available to the controller only after time $t+\TVICD$.
...
For a short measurement interval ($\Delta t<10\,\unit{min}$), it can be assumed to be constant.
Taking into account the carefully aligned reference frame and the official gravitational value for Saarland, we have $\gravityAcc = [0, 0, -9.8107]^\top \tfrac{\unit{m}}{\unit{s}^2}$.

The multicopter is placed in different random orientations while the accelerometer measurements $\aIMU$ and the attitude $\R$ are recorded.
To compensate for the noise $\aIMUN[k]$, we take the mean value $\bar{\tuple{a}}_{\idxText{M}}$ over $10\,\unit{s}$.
For each measurement index $i$ and attitude $\bar{\R}[i]$, we obtain the mean accelerometer measurement $\bar{\tuple{a}}_{\idxText{M}}[i]$.
...
Finally, the multicopter microcontroller receives the measurements through its radio module.
The radio module also handles other transmissions like receiving commands and sending measurements to the groundstation.

Because this chain involves non-realtime elements, the latency at which the configuration measurements are available to the multicopter is \textit{not} constant, but varies by several main sampling steps $\Ts$ in addition to its average value.
...
If the previous assumption \eqref{eq:meanTPC} holds, these signals should be similar up to a shift of the mean delay $\meanTVICD$.
...
The identified average delay is the lag at which the cross-correlation attains its maximum, $\meanTVICD = 0.034\,\unit{s}$.
...
The differential relation between position and accelerometer measurement is given in \eqref{eq:RealizationImuModel}. 
...
To choose reasonable correction gains $\mat{L}_r$, $\mat{L}_v$ and $\mat{L}_a$, we investigate the resulting error dynamics of the estimation algorithm above.
